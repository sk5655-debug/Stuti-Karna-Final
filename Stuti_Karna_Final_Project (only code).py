...
df = df.rename(columns={ #permanently renames the original columns
    ...
    "A1PB1":     "education",
    "A1SHHTOT":  "income",
    "A1PDEPDX":  "depression",    # binary depression diagnosis (0/1)
    "A1PDEPAF":  "dep_score",     # continuous depressed affect score
    "A1SA9U":    "substance_use", # alcohol/drug problem (1=yes, 2=no)
...
df["substance_use"] = df["substance_use"].replace({1.0:1, 2.0:0, -1.0:np.nan, 8.0:np.nan}) #for some reason if I dont run this again my descriptives and bar chart in section 9-10 incorrectly calculate and use the % for substance_use
...
trauma    = df[df["trauma_group"] == "Trauma"] ## the bar chart also kept using the trauma df before I recoded it so I had to add this code to rebuild the data for the bar chart properly.
